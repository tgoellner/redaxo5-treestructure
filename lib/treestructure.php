...
abstract class rex_treestructure {
    ...
    public static function getCategoryHtml($category = 0, $quiet = false) {
    	$output = '';
        ...
        if(empty($category)) {
            $cat['children'] = rex_category::getRootCategories();
            $cat['articles'] = rex_article::getRootArticles();
        }
        else {
            if (! $category instanceof rex_category) {
                $category = rex_category::get(is_object($category) ? $category->getId() : (int) $category);
    ...
    public static function getArticleHtml($article = 0, $quiet = false) {
    	$output = '';
